...
docker cp $CONTAINER_ID:/app/coverage/combined/coverage-summary.json ./coverage/coverage-summary.json
docker cp $CONTAINER_ID:/app/README.md ./README.md

# Set env variables for coverage and readme files
echo "::set-env name=COVERAGE_PATH::$(pwd)/coverage/coverage-summary.json"
